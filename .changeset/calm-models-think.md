...
Report ACP models as canonical `provider:model` IDs and expose reasoning effort through the `thought_level` config option. ACP sessions now persist the canonical model and thought level separately. Existing saved ACP sessions use the old shape and cannot be loaded after this update. The `acp --model` flag keeps its `provider/model/effort` syntax, but the effort must now be one of the advertised thought levels.
